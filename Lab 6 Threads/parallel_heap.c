...
void* computeIntegral(void* argument) {
    ...
    double* curSumPtr = (double*)malloc(sizeof(double));
    *curSumPtr = 0.0;

    for (unsigned long i = arg->start; i < arg->end; ++i) {
        double const x = (double)i * EACH_RECTANGLE_WIDTH + LOWER_BOUND;
        *curSumPtr += 4.0 * (EACH_RECTANGLE_WIDTH * (sqrt(1 - x * x)));
    }

    // pthread_exit() argument can be accessed with pthread_join(,  void** thread_return)
    pthread_exit((void*)curSumPtr);
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < NUM_TESTS; ++i) {
        ...
        struct ThreadArgument threadIntervals[NUM_THREADS];
        ...
        for (int i = 0; i < NUM_THREADS; ++i) {
            ...
            if (i == NUM_THREADS - 1) {
                // if last Thread, add leftovers to end
                end += LEFTOVER_INTERVALS_FOR_LAST_THREAD;
            }

            // printf("Start %lu, End %lu\n", start, end);

            threadIntervals[i] = (struct ThreadArgument){
                .start = start,
                .end = end
            };

            int createThreadStatus = pthread_create(&myThreads[i], NULL, computeIntegral, (void*)&threadIntervals[i]);
        ...
        /* Joining Threads */
        for (int i = 0; i < NUM_THREADS; ++i) {
            double* retValPtr; // pointer that will == heap allocated double ptr
            pthread_join(myThreads[i], (void**)&retValPtr);
